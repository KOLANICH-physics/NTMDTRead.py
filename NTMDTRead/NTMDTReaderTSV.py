...
class NTMDTReaderTSV():
	def __init__(self, fileName, sep="\t"):
		...
		if(self.meta["State"] == "Table"):
			self.data=np.vstack((
				self.readColumnsTSVPart(fileName,"Fw"),
				self.readColumnsTSVPart(fileName,"Bk",usecols=(1))
			))
		elif(self.meta["State"] == "Row"):
			self.data=np.vstack((
				np.linspace(self.meta["Bias X"],self.meta["Bias X"]+self.meta["Scale X"]*self.meta["FwSize"],num=self.meta["FwSize"]),
	...
	def readColumnsTSVPart(self,fileName,partName,usecols=None):
		return np.transpose(np.genfromtxt(
				fileName,
				usecols=usecols,
				skip_header=self.meta.lines[partName+"Size"][1]+1,
				max_rows=self.meta[partName+"Size"],
				delimiter=self.sep,
				invalid_raise=True
			))
